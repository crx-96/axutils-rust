...
- `.github/`：GitHub Actions 工作流目录，维护跨平台格式、构建、测试、Clippy 和发布包检查。
  工作流属于项目质量保障的一部分；仓库使用 GitHub 时应提交，且不得在 workflow 中写入凭据或
  自动执行真实 SMTP 发信。
...
- `docs/release-notes/`：版本发布日志和发布说明草稿目录。涉及公共 API、feature、依赖、MSRV、
  安全边界或兼容性变化时，应同步更新并提交对应发布说明；该目录仅供仓库维护，不进入 crate
  发布包。
- `config/`：本地测试和开发配置目录，整体由 `.gitignore` 忽略。可以通过代码读取其中的测试
  配置，但不得提交 `email-test.toml`、密码、授权码、邮箱地址或其他凭据，也不得将其写入
  命令行、日志或文档。
- `.github/`、`tests/` 和 `docs/release-notes/` 不属于运行时依赖；删除它们会分别失去自动 CI、
  回归测试或发布记录，删除前必须确认对应能力不再需要并同步清理目录引用。
...
- 依赖版本约束必须使用最低兼容版本的 Cargo 默认 caret 写法（例如 `version = "0.11.22"`），不得在 `version` 字段使用等号前缀来精确锁定补丁版本。本项目是 library crate，不提交 `Cargo.lock` 作为依赖版本策略；安全下限和兼容性通过 manifest、MSRV 与无锁 fresh-resolution CI 验证。若未来确有技术原因需要精确版本，必须先取得用户明确确认，并在计划、状态和发布说明中记录原因；当前项目不设此类例外。
...
和 `src/**` 可以随包发布；`develop.md`、`AGENTS.md`、`CLAUDE.md` 和 `docs/skills/**` 仅供仓库开发使用，

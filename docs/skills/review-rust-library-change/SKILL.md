...
description: 当任务需要按 axutils 的库级标准处理 Rust 实现、公共 API、feature/依赖、契约性测试/文档或发布变更时，执行影响分析、安全审查和证据化验收；纯规则、工具、格式或措辞维护不适用。
---

# axutils Rust library 变更审查与验收

本 Skill 集中定义 `axutils` 的设计、实现、审查和交付验收标准，即“什么才算完成”；项目规则
声明按需入口，并保留仓库边界、授权及其他更具体约束，避免重复维护验收细则。工具类定位、
公共导出和能力边界以
[`docs/module-map.md`](../../module-map.md) 为准；[`docs/develop.md`](../../develop.md) 的读取条件见第 2 节。

## 1. 适用范围、术语和证据要求

本 Skill 不是进入仓库后的默认前置读物。任务实际需要依据本项目标准设计、实现、审查或验收以下
任一库级内容时，才适用本标准，并在首次作出相关判断前完整读取：
...
- 以集成测试、feature/API 编译 fixture、依赖边界断言、公开 API 文档、README、`docs/examples/`
  或发布文件定义、验证或审查上述实现与契约，以及相关回归排查或发布前检查。

不得仅因任务提到“审查”“验收”，或仅因目标位于测试、文档、CHANGELOG、项目规则或 Skill 中就
触发本 Skill。纯翻译、简单措辞或格式调整、规则或 Skill 自身维护、CI/开发工具维护、Git/工作区
操作、测试基础设施调整及其他不改变也不判断 Rust library 实现或契约的任务不适用；执行过程中
一旦需要作出上述库级判断，再在判断前完整读取并使用。
...
任务命中第 1 节后，在开始相关实现、审查或验收判断前按顺序建立上下文：
...
- `AGENTS.md` 必须链接本 Skill，并与第 1、2 节的按需触发范围、例外和读取要求保持一致；
- 可跨任务复用的稳定流程只维护在本项目级、工具无关的 Skill，不写入特定 Agent 工具语法；
- 修改本 Skill 时须完整读取目标文件，并检查规则链接、触发描述和 module map 引用；这属于目标文件
  维护，不代表自动套用全部 Rust library 变更验收步骤。`docs/develop.md` 仍按第 2 节处理。Skill、
  规则或纯文档整理本身不提升版本、不写 CHANGELOG；公共实现变更仍按第 7.4 节处理；

//! 配置领域的无状态便捷入口。
//!
//! 启用 `config-async` feature 后，文件读取入口还提供异步版本；异步版本要求调用方
...
impl ConfigUtils {
    ...
    /// use axutils::utils::ConfigUtils;
    ///
    /// let mut path = env::temp_dir();
    /// path.push(format!("axutils-config-utils-doctest-{}.json", process::id()));
    /// File::create(&path)
    ///     .unwrap()
    ...
    ///
    /// 该方法仅在启用 `config-async` feature 时提供，调用方必须在已有 Tokio runtime
    ...
    ///
    /// ```no_run
    /// use axutils::{config::ConfigError, utils::ConfigUtils};
    ///
    ...
    ///
    /// let mut path = env::temp_dir();
    /// path.push(format!("axutils-config-utils-doctest-load-{}.json", process::id()));
    /// File::create(&path)
    ///     .unwrap()
    ///     .write_all(br#"{"port": 8080}"#)
    ///     .unwrap();
    ...
    ///
    /// let mut path = env::temp_dir();
    /// path.push(format!("axutils-config-utils-doctest-explicit-{}.txt", process::id()));
    /// File::create(&path)
    ///     .unwrap()
    ///     .write_all(br#"{"port": 8080}"#)
    ///     .unwrap();
    ...
    ///
    /// 该方法仅在启用 `config-async` feature 时提供，显式格式会覆盖扩展名推断；调用方
    ...
    ///
    /// ```no_run
    /// use axutils::{config::{ConfigError, ConfigFormat}, utils::ConfigUtils};
    ///
    /// async fn example() -> Result<(), ConfigError> {
    ...
    ///
    /// let mut path = env::temp_dir();
    /// path.push(format!("axutils-config-utils-doctest-explicit-typed-{}.txt", process::id()));
    /// File::create(&path)
    ///     .unwrap()
    ///     .write_all(br#"{"port": 8080}"#)
    ///     .unwrap();
...
mod tests {
    ...
    fn load_value_as_and_load_as_use_explicit_format() {
        ...
        fs::write(&path, r#"{"a": 7}"#).expect("write temp file");

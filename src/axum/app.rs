...
/// 调用方通常无需显式写出后两项。类型仅在启用 crate 的 `axum` feature 时公开，
/// 推荐通过 `axutils::axum::AxumApp` 使用。
///
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    /// 仅在 crate 启用 `axum` feature 时可用；调用方若在签名中使用返回类型，
    /// 仍需直接依赖兼容版本的 Axum。
    ///
    /// # Examples
    ///
    ...
    /// # use axutils::axum::*;
    /// # use axutils::axum::*;
    /// let router: axum::Router<String> = AxumApp::<String>::create_router();
    ...
    /// 不 bind 且不访问网络。仅要求 crate 的 `axum` feature，无需额外 provider feature。
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     routing::get(|| async { "ok" }),
    /// );
    /// let _app = AxumApp::from_router(router);
    /// ```
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    /// 网络，也不限制路径长度和路由数量。仅要求 crate 的 `axum` feature。
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// 网络。API 本身只要求 crate 的 `axum` feature；签名中的 Tower trait 由 `axum`
    /// feature 的内部依赖提供，不要求另开`axum-tower` feature。
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// });
    /// let _app = AxumApp::new().route_service("/health", service);
    /// ```
    ...
    /// 网络，且本 crate 不额外限制嵌套深度和路由数量。仅要求 crate 的 `axum` feature。
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    ///     "/users",
    ///     routing::get(|| async { "users" }),
    /// );
    /// let _app = AxumApp::new().nest("/api", api);
    /// ```
    ...
    ///     "/ready",
    ///     routing::get(|| async { "ready" }),
    /// );
    /// let _app = AxumApp::new().merge(extra);
    /// ```
    ...
    /// 它不 bind 或访问网络，且只要求 crate 的 `axum` feature。
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    /// 控制。API 只要求 crate 的 `axum` feature，不要求本 crate 的 `tower` provider
    /// feature。
    ///
    /// # Examples
    ///
    ...
    /// # use axum::extract::Request;
    /// # use axutils::axum::*;
    /// let layer = middleware::from_fn(
    ///     |request: Request, next: middleware::Next| async move {
    ///         next.run(request).await
    ///     },
    /// );
    /// let _app = AxumApp::new().with_layer(layer);
    /// ```
    ...
    /// crate 的 `axum` feature，不要求`axum-tower` feature。
    ///
    /// # Examples
    ///
    /// ```rust,no_run
    ...
    R: FnOnce(Router) -> Router,
{
    ...
    /// # use axum::routing;
    /// # use axutils::axum::*;
    /// async fn handler(extract::State(name): extract::State<String>) -> String {
    ///     name
    /// }
    /// let router = Router::<String>::new()
    ///     .route("/name", routing::get(handler));
    /// let _builder = AxumApp::from_router(router).with_state("axutils".to_owned());
    /// ```
...
impl AxumApp<()> {
    ...
    /// 后续注册操作和 Axum 0.8 决定。仅在 crate 启用 `axum` feature 时可用。

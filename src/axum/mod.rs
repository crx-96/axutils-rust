...
//!
//! 本模块只在 crate 同时启用 `axum` 与 `tokio` feature 时以 `axutils::axum` 公开；
//! 主要类型同时从 crate 根重导出。基础 API 负责在内存中组装路由和配置，并由调用方提供
...
//!
//! Middleware 按 provider feature 精确开放：`tower` 提供并发限制，`tower-http` 提供 CORS、
//! request ID、service timeout、请求体限制和 panic 捕获，`tower-http + tracing` 提供 HTTP trace，
//! `tower_governor` 提供按 peer IP 或未经验证的转发 header 限流。构造 builder 或安装 layer
